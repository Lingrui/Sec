...
def TfidfV(train_df,test_df,split_type):
    ...
    for w in dic:
        ...
        #print(x)
        a.append(x)
    #dic = dic.apply(lambda x: [w.decode("unicode-escape") for w in dic])
    #print(dic)
    f = open ('test.dictionary.txt','wb')
    pickle.dump(a,open('test.dictionary','wb'))
    for temp in a:
        print (temp, file = f)
    #with open('dictionary', 'rb') as dic:
    #    print(pickle.load(dic))
...
def SVD(train_df,test_df,split_type,full_tfidf,train_tfidf,test_tfidf):
    ...
    #print(train_svd)
#print (svd_obj.components_.)
    #np.save("test_SVD.npy",svd_obj.components_)
    #np.save("test_comp.npy",train_svd)
