...
def runXGB(train_X, train_y, test_X, test_y=None, test_X2=None, seed_val=0, child=1, colsample=0.5):
    ...
    '''
    #xgb.dump(model, 'xgb.model.dump', with_stats = TRUE)
    #cat(xgb.dump(model,with_stats = TRUE, dump_format='json'))
...
def tfidf_word(train_df,test_df):
    ...
    for i in xrange(len(weight)):
        for j in xrange(len(full_tfidf)):
            print(full_tfidf[j],weight[i][j])
